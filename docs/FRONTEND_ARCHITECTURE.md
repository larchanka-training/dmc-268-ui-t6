...
**Стек.** React 18.3.1, Vite 5.4.21, TypeScript 5.9.3, pnpm 12, antd 6.6.4, react-diff-view 3.3.3,
Zod 4.6.5, Zustand 5.0.15, TanStack Query 5.103.1, Vitest 3.2.7 (не 4.x: vitest 4 требует Vite ≥ 6,
у нас зафиксирован Vite 5 — переход отложен до бампа Vite, см. §11).
...
Альтернатива — **Mantine 8**: рассматривалась, отклонена, потому что Mantine 9.x (актуальный
на момент решения) требует `react ≥ 19.2`, а React зафиксирован на 18.3.1 (SD, L12; PR #26 бамп
не поднимал). Mantine 8 — не последний мажор линейки, брать его сейчас означало бы объявленно
устаревший пин.
...
flowchart LR
  ...
  C -->|Diff viewType hunks| D["Диффcomponent react-diff-view"]
...
`startedAt` старше 10 минут (`STALE_RUNNING_MS`) — порог совпадает с таймаутом реконсилера SD §13
...
- `mockReviewComments` — 3 комментария (один — с `ruleName`);
...
```json
{
  ...
  "finishedAt": "2026-09-18T11:58:00.000Z",
  "attempt": 1,
...
**Роутер.** Назван **react-router 7** (v8 требует `react ≥ 19.2`, у нас 18.3.1), **не
установлен** — продуктовых экранов в спринте нет, `src/pages/{runs,review}/` — только
плейсхолдеры (`.gitkeep`).
...
Команды: `pnpm test` (запускает `vitest run`, работает уже сейчас). После мержа PR #26
добавляются `pnpm lint`, `pnpm check-types`, `pnpm build` — они настраиваются ролью 4, в этой
ветке не выполняются.
...
antd) и `ResizeObserver` (нужен `Tree` через `@rc-component/virtual-list`) — jsdom 30.0.1 их не
предоставляет.
...
- **Бамп React до 19** — разблокирует Mantine 9 (§3), react-router 8 (§9), vitest 4 через Vite 7
  (шапка); не в скоупе роли 5.
...
| Инспектор прогонов: шапка, дерево, схлопывание, статусы queued/running/completed/failed | §7; код — `src/widgets/run-inspector/` (статусов фактически 7, см. §4, Ф-decision по расхождениям) |
