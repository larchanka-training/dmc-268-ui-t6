...
describe('runApi', () => {
  ...
  it('accepts a completed run.updated event and rejects an unknown status', () => {
    expect(runApi.stream.event.safeParse({ runId: UUID, status: 'completed' }).success).toBe(true)
    expect(runApi.stream.event.safeParse({ runId: UUID, status: 'succeeded' }).success).toBe(false)
  })
